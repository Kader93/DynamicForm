...
class MembreForm extends Form
{
    ...
    public function isValidElement($element)
    {
        ...
        if ($elementCheck['name'] === NULL) {
            return FALSE;
        }

        if (in_array(strtolower($elementCheck['type']), $this->allowedElement)
            || in_array(strtolower($elementCheck['attributes']['type']), $this->allowedElement)) {
            return TRUE;
        }
        return FALSE;
    }
